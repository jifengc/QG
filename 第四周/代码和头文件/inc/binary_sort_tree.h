 ...
 */
Status BST_insert(BinarySortTreePtr p, ElemType value)
{
    ...
    while (1)
    {
        ...
        {
            p->root->value = value;
            p->root->left = (Node*)malloc(sizeof(Node));
            p->root->right = (Node*)malloc(sizeof(Node));
            p->root->left->value =p->root->right->value = 0x7fffffff;
            break;
        }else{
            if (value < p->root->value)
            {
                p->root = p->root->left;
            }else if (value == p->root->value)
            {
                printf("该值已存在于树中！\n");
                return failed;
            }else if (value > p->root->value)
